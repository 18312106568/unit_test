...
package com.creditcloud.jpa.unit_test.bigData.kafka.generator;

import com.creditcloud.jpa.unit_test.bigData.kafka.dfa.EventTypeAndState;
...
import com.creditcloud.jpa.unit_test.bigData.kafka.event.EventType;
 ...
 * 事件生成器。发电机内部保持一系列的状态
 * 计算机（地址和当前关联状态）并返回转换事件
 * 从那些国家机器。每次下一个事件是生成器时
 * 生成器选择一个随机状态机并在其上创建一个随机转换
 * 状态机。
 * <p>生成器随机添加新的状态机，并将状态机删除为
 * 一旦他们到达终点。此实现保持
 * 同时有1000台状态机。
 */
